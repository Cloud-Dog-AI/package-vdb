...
class _TimeoutClient:
    async def post(self, url: str, **kwargs: Any) -> _FakeResponse:
        _ = (url, kwargs)
        return _FakeResponse(200, {"job_id": "job-timeout"})
...
@pytest.mark.asyncio
async def test_ut3_5_async_runner_raises_parser_timeout(monkeypatch: pytest.MonkeyPatch) -> None:
    values = [0.0, 3.0, 3.0]
    state = {"index": 0}
    ...
    runner = AsyncParseRunner(_TimeoutClient(), poll_interval=0.01, max_wait=1.0, timeout=5.0)
