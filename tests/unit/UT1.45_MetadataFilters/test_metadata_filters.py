...
def test_filter_translation_includes_metadata_pack_management_fields() -> None:
    query = filter_to_backend_query(
        MetadataFilter(
            tenant_id="tenant-a",
...
def test_filter_matching_supports_metadata_pack_management_fields() -> None:
    metadata = {
        "tenant_id": "tenant-a",
        "dataset_id": "dataset-a",
        "collection_id": "collection-a",
    ...
    assert not matches_metadata(metadata, {"language": "fr"})
