...
@pytest.mark.asyncio
async def test_st3_3_marker_mcp_extracts_image_artefacts_live(
    ...
    for attempt in range(3):
        try:
            ir = await provider.parse_bytes(
                ...
                options={"output_format": "markdown", "paginate_output": True, "page_range": 0},
            )
            break
    ...
    if ir is None:
        raise last_error if last_error is not None else RuntimeError("Marker image artefact parse failed without error")

    assert ir.provider_id == "marker_mcp"
    assert len(ir.artefact_refs) >= 1
    assert ir.quality.get("images_count", 0.0) >= 1.0
