...
@pytest.mark.asyncio
async def test_st3_2_marker_mcp_async_mode_fallback_live(
    ...
    phases = {(event.get("event"), event.get("phase")) for event in events}
    assert ("heartbeat", "start") in phases
    assert ("heartbeat", "done") in phases
