...
@pytest.mark.asyncio
async def test_pt2_3_ocr_auto_trigger_ratio_is_reported(
    services: dict[str, dict[str, object]],
    corpus_entries: list[dict[str, object]],
) -> None:
    enabled = enabled_parser_ids(services)
    if not enabled:
        pytest.fail("No parser providers are enabled for PT2 OCR ratio benchmarks", pytrace=False)

    matrix = await evaluate_enabled_providers(services=services, corpus_entries=corpus_entries)
    for provider_id in enabled:
        ratio = float(matrix[provider_id]["ocr_auto_trigger_ratio"])
