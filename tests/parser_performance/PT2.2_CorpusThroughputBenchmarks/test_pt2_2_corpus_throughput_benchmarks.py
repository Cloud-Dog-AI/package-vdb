...
@pytest.mark.asyncio
async def test_pt2_2_throughput_measured_for_enabled_parsers(
    services: dict[str, dict[str, object]],
    corpus_entries: list[dict[str, object]],
) -> None:
    enabled = enabled_parser_ids(services)
    if not enabled:
        pytest.fail("No parser providers are enabled for PT2 throughput benchmarks", pytrace=False)

    matrix = await evaluate_enabled_providers(services=services, corpus_entries=corpus_entries)
    for provider_id in enabled:
        throughput = float(matrix[provider_id]["throughput_pages_per_second"])
