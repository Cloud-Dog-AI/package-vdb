...
@pytest.mark.asyncio
async def test_pt1_4_marker_mcp_corpus_quality_matrix(
    services: dict[str, dict[str, object]],
    corpus_entries: list[dict[str, object]],
) -> None:
    marker = services["marker_mcp"]
    if not marker.get("enabled"):
        pytest.skip("Marker-MCP parser tests disabled; set MARKER_MCP_ENABLED=true to enforce")

    summary = await evaluate_provider(provider_id="marker_mcp", services=services, corpus_entries=corpus_entries)
