...
@pytest.mark.asyncio
async def test_pt3_6_table_extraction_metrics_present(
    ...
    report, _ = await run_comparison(
        ...
        report_basename="pt3_6_table_extraction_comparison",
    )
    ok_cases = [case for case in report.get("cases", []) if isinstance(case, dict) and case.get("status") == "ok"]
    assert len(ok_cases) >= 1
    for case in ok_cases:
        assert "table_count" in case
        assert int(case.get("table_count", 0) or 0) >= 0
