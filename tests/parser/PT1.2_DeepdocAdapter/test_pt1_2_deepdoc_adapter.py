...
@pytest.mark.asyncio
async def test_pt1_2_deepdoc_corpus_quality_matrix(
    services: dict[str, dict[str, object]],
    corpus_entries: list[dict[str, object]],
) -> None:
    deepdoc = services["deepdoc"]
    if not deepdoc.get("enabled"):
        pytest.skip("DeepDoc parser tests disabled; set DEEPDOC_ENABLED=true to enforce")

    summary = await evaluate_provider(provider_id="deepdoc", services=services, corpus_entries=corpus_entries)
