...
@pytest.mark.asyncio
async def test_pt1_8_transformers_corpus_quality_matrix(
    services: dict[str, dict[str, object]],
    corpus_entries: list[dict[str, object]],
) -> None:
    transformers = services["transformers"]
    if not transformers.get("enabled"):
        pytest.skip("Transformers parser tests disabled; set TRANSFORMERS_ENABLED=true to enforce")

    summary = await evaluate_provider(provider_id="transformers", services=services, corpus_entries=corpus_entries)
