...
@pytest.mark.asyncio
async def test_pt1_1_mineru_corpus_quality_matrix(
    services: dict[str, dict[str, object]],
    corpus_entries: list[dict[str, object]],
) -> None:
    mineru = services["mineru"]
    if not mineru.get("enabled"):
        pytest.skip("MinerU parser tests disabled; set MINERU_ENABLED=true to enforce")

    summary = await evaluate_provider(provider_id="mineru", services=services, corpus_entries=corpus_entries)
