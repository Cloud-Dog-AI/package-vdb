...
@pytest.mark.asyncio
async def test_opensearch_hybrid(opensearch_ready):
    ...
    n = f"cloud_dog_ai_opensearch_hybrid_{uuid.uuid4().hex[:10]}"
    try:
        await a.delete_collection(n)
        await a.create_collection(CollectionSpec(name=n, embedding_dim=4))
        await a.add_documents(n, ["alpha text"], [{}], ["a"])
        out = await a.search(n, "alpha", 1, search_options={"hybrid_enabled": True})
