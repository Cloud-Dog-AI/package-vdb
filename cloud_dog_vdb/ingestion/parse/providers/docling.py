...
class DoclingParserProvider(ParserProvider):
    ...
    async def parse_bytes(
        ...
        *,
        ...
        if not args:
            raise BackendUnavailableError("docling command is not configured")
        with TemporaryDirectory(prefix="cdvdb-docling-") as tmp:
            ...
            if proc.returncode != 0:
                raise InvalidRequestError(f"docling command failed: {stderr.decode('utf-8', errors='replace')[:200]}")
