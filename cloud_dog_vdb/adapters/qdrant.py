...
class QdrantAdapter(VDBAdapter):
    ...
    async def create_collection(self, spec: CollectionSpec) -> dict:
        ...
        self._dims[spec.name] = spec.embedding_dim
        await self._wait_for_collection_ready(spec.name)
        return {"name": spec.name, "status": "created"}

    async def _wait_for_collection_ready(self, name: str) -> None:
        """Wait until qdrant reports the collection as addressable."""
        deadline = asyncio.get_running_loop().time() + max(float(self.config.timeout_seconds), 1.0)
        ...
        while True:
            ...
            if asyncio.get_running_loop().time() >= deadline:
                resp.raise_for_status()
                raise RuntimeError(f"Qdrant collection {name!r} did not become ready")
            await asyncio.sleep(0.1 if last_status == 404 else 0.25)
    ...
    async def add_documents(
        ...
        for i, content in enumerate(documents):
            ...
            points.append({"id": doc_id, "vector": embeddings[i], "payload": payload})
        ...
        if resp.status_code == 404:
            deadline = asyncio.get_running_loop().time() + max(float(self.config.timeout_seconds), 1.0)
            while resp.status_code == 404 and asyncio.get_running_loop().time() < deadline:
    ...
    async def search(
        ...
        body: dict[str, Any] = {
            "vector": await self._embed_text(query or "*", dim),
            "limit": n_results,
            "with_payload": True,
        }
        if filter:
        ...
        resp = await self._client.put(
            ...
            json={
                "points": [
                    {
                        "id": point_id,
                        "vector": await self._embed_text(content, dim),
