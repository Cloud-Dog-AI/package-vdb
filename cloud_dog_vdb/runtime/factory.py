...
def _embedding_config_from_dict(raw: dict[str, Any]) -> dict[str, Any]:
    """Extract shared embedding settings from resolved runtime config."""
    if not isinstance(raw, dict):
        return {}
    provider_id = str(raw.get("provider", "")).strip().lower()
    provider_cfg = raw.get(provider_id)
    if not isinstance(provider_cfg, dict):
        provider_cfg = {}
    return {
        "embedding_provider_id": provider_id,
        "embedding_base_url": str(provider_cfg.get("base_url", raw.get("base_url", ""))),
        "embedding_api_key": str(provider_cfg.get("api_key", raw.get("auth_token", ""))),
        "embedding_model": str(provider_cfg.get("model", raw.get("model", ""))),
        "embedding_timeout_seconds": float(
            provider_cfg.get("timeout_seconds", raw.get("timeout_seconds", 30.0)) or 30.0
        ),
    }


def provider_config_with_embeddings(
    provider_id: str, raw: dict[str, Any], embeddings: dict[str, Any]
) -> ProviderConfig:
    """Build provider config with embedding settings from resolved runtime config."""
    base = asdict(provider_config_from_dict(provider_id, raw))
    return ProviderConfig(**(base | _embedding_config_from_dict(embeddings)))
...
def build_runtime_client(config: dict[str, Any]) -> VDBClient:
    ...
    for provider_id in ("chroma", "qdrant", "weaviate", "opensearch", "pgvector", "infinity"):
        ...
        if not isinstance(raw, dict) or not raw.get("enabled", False):
            continue
        provider_config = provider_config_with_embeddings(
            provider_id,
            raw,
            embeddings if isinstance(embeddings, dict) else {},
        )
